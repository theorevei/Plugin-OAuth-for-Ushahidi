	...
	if (!class_exists('Utilisateur'))
	{
		class	Utilisateur
		{
			...
			public function UpdateGoogleId($google_id, $autocommit = true)
			{
				...
				{
					$sql = 'UPDATE '.$this->table_prefix.strtolower($this->m_class).' SET google_id = '.$google_id;
			...
			public function UpdateTwitterId($twitter_id, $autocommit = true)
			{
				...
				{
					$sql = 'UPDATE '.$this->table_prefix.strtolower($this->m_class).' SET twitter_id = '.$twitter_id;
			...
			{
				$class = 'utilisateur';
				$sql = 'SELECT * FROM '.$this->table_prefix.strtolower($class);
